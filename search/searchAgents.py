...
def euclideanDistance(cur,goal):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if next_point in self.corners:
                    if next_point not in visited_corners_list:
                        visited_corners_list.append(next_point)
                successors.append(((next_point,tuple(visited_corners_list)),action,1))
...
def foodHeuristic(state, problem):
    ...
    for i in foodGrid.asList():
        maxDistance=max(maxDistance,mazeDistance(position,i,problem.startingGameState))
    return maxDistance
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        return search.bfs(problem)#my code
...
def calculateHeuristic(cur,unsearch_list):

    heuristic = 0
    while len(unsearch_list)>0:
        min_distance, min_point = reduce(min,map(lambda x:(util.manhattanDistance(cur,x),x),unsearch_list))
        heuristic = heuristic + min_distance
        cur = min_point
        unsearch_list.remove(min_point)
